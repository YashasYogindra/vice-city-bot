"""Seed the SinBot database with demo-ready state.
...
Usage:
    ...
    py -3.11 tools/seed_demo.py --db-path path/to/sinbot.db
"""
...
# Add project root to path so we can import sinbot modules
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from sinbot.constants import GANGS
from sinbot.models.events import CityEventEffect
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed SinBot demo state")
    parser.add_argument("--db-path", type=str, default="sinbot.db", help="Path to the SQLite database")
