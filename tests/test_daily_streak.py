...
class DailyStreakTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.bot = SimpleNamespace(
            config=SimpleNamespace(disable_cooldowns=False),
            repo=self.repo,
    ...
    async def test_first_daily_claim_starts_streak(self) -> None:
        ...
        with (
            patch("sinbot.services.city.utcnow", return_value=claim_time),
            patch("sinbot.repositories.game_repository.utcnow", return_value=claim_time),
        ):
    ...
    async def test_consecutive_claim_increases_streak_and_reward(self) -> None:
        ...
        with (
            patch("sinbot.services.city.utcnow", return_value=first_claim),
            patch("sinbot.repositories.game_repository.utcnow", return_value=first_claim),
        ):
            await self.service.claim_daily_reward(1, 100)
        with (
            patch("sinbot.services.city.utcnow", return_value=second_claim),
            patch("sinbot.repositories.game_repository.utcnow", return_value=second_claim),
        ):
            await self.service.claim_daily_reward(1, 100)
    ...
    async def test_missed_day_resets_streak(self) -> None:
        ...
        with (
            ...
            await self.service.claim_daily_reward(1, 100)
        with (
            patch("sinbot.services.city.utcnow", return_value=reset_claim),
            patch("sinbot.repositories.game_repository.utcnow", return_value=reset_claim),
        ):
            embed = await self.service.claim_daily_reward(1, 100)
    ...
    async def test_daily_claim_uses_local_timezone_boundaries(self) -> None:
        ...
        with (
            ...
            await self.service.claim_daily_reward(1, 100)
        with (
            patch("sinbot.services.city.utcnow", return_value=blocked_claim),
            patch("sinbot.repositories.game_repository.utcnow", return_value=blocked_claim),
        ):
            with self.assertRaises(InvalidStateError):
                await self.service.claim_daily_reward(1, 100)
        with (
            patch("sinbot.services.city.utcnow", return_value=next_local_day_claim),
            patch("sinbot.repositories.game_repository.utcnow", return_value=next_local_day_claim),
        ):
            await self.service.claim_daily_reward(1, 100)
