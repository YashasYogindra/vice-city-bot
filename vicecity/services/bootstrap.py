...
from vicecity.constants import CITY_CHANNELS, GANGS, TURF_NAMES
...
class BootstrapService:
    ...
    async def _ensure_owner_has_role(self, owner: discord.Member, role: discord.Role) -> None:
        if role not in owner.roles:
            await owner.add_roles(role, reason="Vice City OS mayor setup")
    ...
    async def _ensure_city_channels(self, guild: discord.Guild) -> dict[str, int]:
        ...
        for setting_key, channel_name in CITY_CHANNELS.items():
            ...
            if channel is None:
                channel = await guild.create_text_channel(channel_name, reason="Vice City OS setup")
        ...
        for gang in GANGS:
            ...
            if channel is None:
                channel = await guild.create_text_channel(
                    ...
                    reason="Vice City OS gang setup",
                )
            else:
