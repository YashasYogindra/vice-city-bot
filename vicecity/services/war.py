...
from vicecity.constants import WAR_DURATION_SECONDS, WAR_MOBILIZATION_COST, WAR_RANDOM_MAX, WAR_RANDOM_MIN
...
class WarService:
    ...
    async def commit(self, member: "discord.Member", mode: str) -> dict:
        ...
        if player is None or not player["is_joined"]:
            raise InvalidStateError("You need to join Vice City first.")
        if player["rank"] not in ("Soldier", "Lieutenant", "Capo", "Boss"):
