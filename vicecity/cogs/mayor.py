...
from vicecity.utils.checks import require_city_admin, require_mayor
...
class MayorCog(commands.Cog):
    ...
    @classmethod
    async def create(cls, bot: "ViceCityBot") -> "MayorCog":
        return cls(bot)
    ...
    async def mayor_tax(self, ctx: commands.Context, tax_rate: int) -> None:
        ...
        await ctx.send(
            embed=self.bot.embed_factory.standard("Tax Updated", f"Vice City tax is now **{tax_rate}%**."),
            view=QuickActionsView(self.bot, ctx.author.id),
        )
    ...
    async def city(self, ctx: commands.Context) -> None:
        await ctx.send(
            embed=self.bot.embed_factory.standard(
                "City Commands",
                "Use `/city event` to check the live city event, or `/city sync` (Mayor only) to re-run setup.",
            ),
            view=QuickActionsView(self.bot, ctx.author.id),
        )

    @city.command(name="sync")
    @require_mayor()
    async def city_sync(self, ctx: commands.Context) -> None:
        await self.bot.bootstrap_service.ensure_guild_setup(ctx.guild)  # type: ignore[arg-type, union-attr]
        await self.bot.city_service.refresh_wanted_board(ctx.guild.id)  # type: ignore[union-attr]
        await self.bot.city_service.refresh_vault(ctx.guild.id)  # type: ignore[union-attr]
        await self.bot.heat_service.rehydrate_active_jails(ctx.guild.id)  # type: ignore[union-attr]
        await self.bot.war_service.rehydrate_active_wars(ctx.guild.id)  # type: ignore[union-attr]
        await self.bot.heist_service.rehydrate_active_heists(ctx.guild.id)  # type: ignore[union-attr]
        await ctx.send(
            embed=self.bot.embed_factory.success("City Synced", "Vice City setup and runtime recovery have been refreshed."),
            view=QuickActionsView(self.bot, ctx.author.id),
        )
    ...
    async def city_event_trigger(self, ctx: commands.Context, event_key: str) -> None:
        ...
        embed = self.bot.embed_factory.success(
            "Event Triggered",
            f"**{definition.name}** is now live in Vice City until {discord.utils.format_dt(event.ends_at, style='R')}.",
        )
        await ctx.send(embed=embed, view=QuickActionsView(self.bot, ctx.author.id))
