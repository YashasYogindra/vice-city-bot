class SinBotError(Exception):
    """Base exception for the bot."""


class InsufficientFundsError(SinBotError):
...
class ConcurrentActionError(SinBotError):
    """Raised when a user tries to run a duplicate action concurrently."""


class HeistDMValidationError(SinBotError):
...
class NotFoundError(SinBotError):
    """Raised when a requested game entity is missing."""


class InvalidStateError(SinBotError):
