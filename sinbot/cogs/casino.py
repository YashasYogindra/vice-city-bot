...
from sinbot.views.action_hub import CasinoSelectView, QuickActionsView
...
class CasinoCog(commands.Cog):
    ...
    @classmethod
    async def create(cls, bot: "SinBot") -> "CasinoCog":
        return cls(bot)
    ...
    async def casino_slots(self, ctx: commands.Context, amount: int) -> None:
        ...
        if self.bot.visual_service is not None:
            is_win = embed.color == discord.Color(0xFFD700)
            if is_win and gifs.CASINO_WIN:
                embed.set_image(url=gifs.CASINO_WIN)
            elif not is_win and gifs.CASINO_LOSS:
                embed.set_image(url=gifs.CASINO_LOSS)
            else:
                media_key = "slots_win" if is_win else "slots_loss"
    ...
    async def casino_flip(self, ctx: commands.Context, amount: int, choice: Literal["heads", "tails"]) -> None:
        ...
        is_win = "won" in embed.description.lower()
        if is_win and gifs.CASINO_WIN:
            embed.set_image(url=gifs.CASINO_WIN)
        elif not is_win and gifs.CASINO_LOSS:
            embed.set_image(url=gifs.CASINO_LOSS)
    ...
    async def casino_duel(self, ctx: commands.Context, opponent: discord.Member, amount: int) -> None:
        ...
        if gifs.CASINO_DUEL:
            embed.set_image(url=gifs.CASINO_DUEL)
        await ctx.send(embed=embed, view=QuickActionsView(self.bot, ctx.author.id))
