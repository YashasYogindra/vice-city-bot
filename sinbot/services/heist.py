...
from sinbot.constants import HEIST_EXECUTION_SECONDS, HEIST_PLANNING_SECONDS
from sinbot.exceptions import HeistDMValidationError, InvalidStateError
from sinbot.utils.time import isoformat, parse_datetime, utcnow
from sinbot.views.action_hub import HeistRoleSelectView
...
class HeistService:
    HACKER_TASK_SECONDS = 25
    HACKER_COUNTDOWN_MARKS: tuple[int, ...] = (15, 8, 3)
    INSIDE_WINDOW_SECONDS = 2
    INSIDE_PREP_MARKS: tuple[int, ...] = (10, 5, 2)
    INSIDE_TRIGGER_WORDS: tuple[str, ...] = (
        "NIGHTFALL",
        "SILENCER",
        "GHOSTKEY",
        "BLUELIGHT",
    ...
    def __init__(self, bot: "SinBot") -> None:
        ...
        self.heist_status_channels: dict[int, int] = {}
    ...
    async def join_role(self, member: discord.Member, role_name: str) -> dict:
        ...
        if player is None or not player["is_joined"]:
            raise InvalidStateError("You need a city profile first.")
        gang = await self.bot.repo.get_gang(player["gang_id"])
        if gang is None:
    ...
    async def launch_heist(self, boss: discord.Member, *, status_channel_id: int | None = None) -> dict:
        player = await self.bot.repo.get_player(boss.guild.id, boss.id)
        if player is None or player["rank"] != "Boss":
        ...
        hacker_code = "".join(self.random.sample(string.ascii_uppercase + string.digits, 8))
        hacker_digit_sum = sum(int(ch) for ch in hacker_code if ch.isdigit())
        hacker_answer = f"{hacker_code[::-1]}-{hacker_digit_sum:02d}"
        correct_route = self.random.choice(["A", "B", "C"])
        scanner_count = await self.bot.repo.get_inventory_item(boss.guild.id, heist["driver_id"], "scanner")
        scanner_hint = ""
        if scanner_count > 0:
            await self.bot.repo.adjust_inventory(boss.guild.id, heist["driver_id"], "scanner", -1)
            scanner_hint = f"\n\nSCANNER PING: Route **{correct_route}** currently has the lowest police density."
        inside_trigger = self.random.choice(self.INSIDE_TRIGGER_WORDS)
        inside_window_start = utcnow() + timedelta(seconds=self.random.randint(18, 45))
        inside_window_end = inside_window_start + timedelta(seconds=self.INSIDE_WINDOW_SECONDS)
        treasury_snapshot = (await self.bot.repo.ensure_guild_settings(boss.guild.id))["treasury_balance"]
        await self.bot.city_service.adjust_violence(boss.guild.id, 10)  # type: ignore[union-attr]
        heist = await self.bot.repo.update_heist(
            ...
            hacker_prompt=(
                "VAULT CIPHER (HARD):\n"
                f"Main code: **{hacker_code}**\n"
                "Rule: Reverse the code and append the sum of all digits as two digits.\n"
                f"Example format: **{hacker_code[::-1]}-{hacker_digit_sum:02d}**\n"
                f"You have {self.HACKER_TASK_SECONDS} seconds. Countdown updates will follow."
            ),
            ...
            inside_prompt=(
                "INSIDE MAN REACTION CHECK:\n"
                "A random trigger message will appear later in this DM.\n"
                f"Reply token: {inside_trigger}\n"
                f"When the trigger appears, you have only {self.INSIDE_WINDOW_SECONDS} seconds to send that token.\n"
                "Any early or late answer fails the camera cut."
            ),
        ...
        if status_channel_id is not None:
            self.heist_status_channels[heist["id"]] = status_channel_id
        await self._send_heist_prompts(boss.guild, heist, is_reminder=False)
        self._schedule_role_challenges(heist)
    ...
    async def capture_dm_response(self, message: discord.Message) -> bool:
        ...
        now = utcnow()
        if role == "hacker":
            hacker_deadline = self._hacker_deadline(heist)
            in_time = hacker_deadline is None or now <= hacker_deadline
            success = in_time and message_text.upper() == (heist.get("hacker_answer") or "").upper()
        elif role == "driver":
            success = message_text.upper()[:1] == (heist.get("driver_answer") or "").upper()
        else:
            inside_start = parse_datetime(heist.get("inside_window_start"))
            inside_end = parse_datetime(heist.get("inside_window_end"))
            expected_inside_token = self._extract_inside_token(heist.get("inside_prompt"))
            success = (
                message_text.upper() == expected_inside_token.upper()
        ...
        await self.bot.repo.update_heist(
            heist["id"],
            **{
                response_field: message_text,
                f"{role}_success": 1 if success else 0,
            },
        )
        response_detail = "Your heist response is locked in."
        if role == "hacker":
            hacker_deadline = self._hacker_deadline(heist)
            if hacker_deadline is not None and now > hacker_deadline:
                response_detail = "Your response was recorded after the hacker timer expired."
            elif not success:
                response_detail = "Your response was recorded, but the vault cipher answer was incorrect."
        elif role == "inside":
            inside_start = parse_datetime(heist.get("inside_window_start"))
            inside_end = parse_datetime(heist.get("inside_window_end"))
            if inside_start is not None and now < inside_start:
                response_detail = "Your response was early. The camera trigger had not fired yet."
            elif inside_end is not None and now > inside_end:
                response_detail = "Your response was late. The 2-second inside window had closed."
            elif not success:
                response_detail = "Your response was recorded, but the trigger token did not match."
        elif not success:
            response_detail = "Your response was recorded, but it may not have been the right move."
        await message.channel.send(
            embed=(
                self.bot.embed_factory.success("Task Logged", response_detail)
                if success
                else self.bot.embed_factory.danger("Task Logged", response_detail)
            )
        )
        return True
    ...
    async def resolve_heist(self, heist_id: int) -> None:
        ...
        if successes == 3:
            ...
            medkit_saved = 0
            for user_id in crew_ids:
                medkits = await self.bot.repo.get_inventory_item(guild.id, user_id, "medkit")
                if medkits > 0:
                    await self.bot.repo.adjust_inventory(guild.id, user_id, "medkit", -1)
                    await self.bot.heat_service.apply_heat(guild.id, user_id, 2, reason="Failed Casino Job escape")  # type: ignore[union-attr]
                    medkit_saved += 1
                    continue
            ...
            if medkit_saved:
                outcome_description = (
                    "The crew botched the job. "
                    f"**{medkit_saved}** member(s) burned a medkit and escaped jail; the rest were locked up for two hours."
                )
            else:
        ...
        if channel is not None:
            if successes >= 2 and gifs.HEIST_SUCCESS:
                card_embed.set_image(url=gifs.HEIST_SUCCESS)
            elif successes < 2 and gifs.HEIST_BUSTED:
                card_embed.set_image(url=gifs.HEIST_BUSTED)
            if card_file is not None:
                await channel.send(embed=card_embed, file=card_file)
            else:
                await channel.send(embed=card_embed)
        status_channel_id = self.heist_status_channels.pop(heist_id, None)
        if status_channel_id is not None:
            status_channel = guild.get_channel(status_channel_id)
            if isinstance(status_channel, discord.TextChannel):
                confirmation = (
                    self.bot.embed_factory.success("Heist Result Confirmed", outcome_description)
                    if successes >= 2
                    else self.bot.embed_factory.danger("Heist Result Confirmed", outcome_description)
                )
                confirmation.add_field(name="Crew Tasks Cleared", value=f"{successes}/3", inline=True)
                confirmation.add_field(name="Payout", value=str(payout_total), inline=True)
                await status_channel.send(embed=confirmation)
        for user_id in crew_ids:
    ...
    async def cancel_heist(self, heist_id: int, reason: str, *, notify_boss: bool, status: str = "cancelled") -> None:
        ...
        if status_channel_id is not None:
            status_channel = guild.get_channel(status_channel_id)
            if isinstance(status_channel, discord.TextChannel):
                await status_channel.send(embed=self.bot.embed_factory.danger("Heist Cancelled", reason))
        await self.cleanup_planning_channel(guild, heist)
        if notify_boss:
        ...
        for user_id in user_ids:
            ...
            try:
                await member.send(
                    embed=self.bot.embed_factory.standard(
                        "Heist DM Check",
                        "SinBot is verifying that your DMs are open for the Casino Job.",
                    )
                )
            except discord.HTTPException:
                failed.append(user_id)
    ...
    def _hacker_deadline(self, heist: dict) -> datetime | None:
        execution_deadline = parse_datetime(heist.get("execution_deadline"))
        if execution_deadline is None:
            return None
        launch_time = execution_deadline - timedelta(seconds=HEIST_EXECUTION_SECONDS)
        return launch_time + timedelta(seconds=self.HACKER_TASK_SECONDS)

    def _extract_inside_token(self, inside_prompt: str | None) -> str:
        if not inside_prompt:
            return "!CUT"
        marker = "reply token:"
        lower = inside_prompt.lower()
        idx = lower.find(marker)
        if idx == -1:
            return "!CUT"
        token_part = inside_prompt[idx + len(marker):].strip(" :\n\t")
        token = token_part.splitlines()[0].strip()
        return token or "!CUT"

    def _schedule_role_challenges(self, heist: dict) -> None:
        heist_id = heist["id"]
        now = utcnow()
        hacker_deadline = self._hacker_deadline(heist)
        if hacker_deadline is not None:
            for seconds_left in self.HACKER_COUNTDOWN_MARKS:
                run_at = hacker_deadline - timedelta(seconds=seconds_left)
                if run_at > now:
                    self.bot.scheduler.add_job(
                        self._send_hacker_countdown,
                        "date",
                        run_date=run_at,
                        id=f"heist-hacker-countdown:{heist_id}:{seconds_left}",
                        replace_existing=True,
                        kwargs={"heist_id": heist_id, "seconds_left": seconds_left},
                    )
            if hacker_deadline > now:
                self.bot.scheduler.add_job(
                    self._lock_hacker_timeout,
                    "date",
                    run_date=hacker_deadline,
                    id=f"heist-hacker-timeout:{heist_id}",
                    replace_existing=True,
                    kwargs={"heist_id": heist_id},
                )
        ...
        if inside_start is not None:
            for seconds_left in self.INSIDE_PREP_MARKS:
                run_at = inside_start - timedelta(seconds=seconds_left)
                if run_at > now:
                    self.bot.scheduler.add_job(
                        self._send_inside_countdown,
                        "date",
                        run_date=run_at,
                        id=f"heist-inside-prep:{heist_id}:{seconds_left}",
                        replace_existing=True,
                        kwargs={"heist_id": heist_id, "seconds_left": seconds_left},
                    )
            if inside_start > now:
                self.bot.scheduler.add_job(
                    self._send_inside_trigger,
                    "date",
                    run_date=inside_start,
                    id=f"heist-inside-trigger:{heist_id}",
                    replace_existing=True,
                    kwargs={"heist_id": heist_id},
                )
        if inside_end is not None and inside_end > now:
            self.bot.scheduler.add_job(
                self._lock_inside_timeout,
                "date",
                run_date=inside_end,
                id=f"heist-inside-timeout:{heist_id}",
                replace_existing=True,
                kwargs={"heist_id": heist_id},
            )

    async def _send_hacker_countdown(self, heist_id: int, seconds_left: int) -> None:
        heist = await self.bot.repo.get_heist(heist_id)
        if heist is None or heist.get("status") != "executing" or heist.get("hacker_response"):
            return
        guild = await self.bot.city_service.get_guild(heist["guild_id"])  # type: ignore[union-attr]
        member = guild.get_member(heist.get("hacker_id"))
        if member is None:
            return
        try:
            await member.send(
                embed=self.bot.embed_factory.danger(
                    "Hacker Timer",
                    f"Vault cipher lock rotates in **{seconds_left} seconds**.",
                )
            )
        except discord.HTTPException:
            pass

    async def _lock_hacker_timeout(self, heist_id: int) -> None:
        heist = await self.bot.repo.get_heist(heist_id)
        if heist is None or heist.get("status") != "executing" or heist.get("hacker_response"):
            return
        await self.bot.repo.update_heist(heist_id, hacker_response="[timeout]", hacker_success=0)
        guild = await self.bot.city_service.get_guild(heist["guild_id"])  # type: ignore[union-attr]
        member = guild.get_member(heist.get("hacker_id"))
        if member is None:
            return
        try:
            await member.send(embed=self.bot.embed_factory.danger("Hacker Task Failed", "Timer expired before the vault answer was submitted."))
        except discord.HTTPException:
            pass

    async def _send_inside_countdown(self, heist_id: int, seconds_left: int) -> None:
        heist = await self.bot.repo.get_heist(heist_id)
        if heist is None or heist.get("status") != "executing" or heist.get("inside_response"):
            return
        guild = await self.bot.city_service.get_guild(heist["guild_id"])  # type: ignore[union-attr]
        member = guild.get_member(heist.get("inside_id"))
        if member is None:
            return
        try:
            await member.send(
                embed=self.bot.embed_factory.standard(
                    "Inside Timer",
                    f"Trigger signal incoming in **{seconds_left} seconds**. Stay ready.",
                )
            )
        except discord.HTTPException:
            pass

    async def _send_inside_trigger(self, heist_id: int) -> None:
        heist = await self.bot.repo.get_heist(heist_id)
        if heist is None or heist.get("status") != "executing" or heist.get("inside_response"):
            return
        guild = await self.bot.city_service.get_guild(heist["guild_id"])  # type: ignore[union-attr]
        ...
        if member is None:
            return
        token = self._extract_inside_token(heist.get("inside_prompt"))
        inside_end = parse_datetime(heist.get("inside_window_end"))
        end_text = discord.utils.format_dt(inside_end, style="R") if inside_end is not None else "in 2 seconds"
        try:
            await member.send(
                embed=self.bot.embed_factory.danger(
                    "Inside Trigger LIVE",
                    f"Type **{token}** right now. Window closes {end_text}.",
                )
            )
        except discord.HTTPException:
            pass

    async def _lock_inside_timeout(self, heist_id: int) -> None:
        heist = await self.bot.repo.get_heist(heist_id)
        if heist is None or heist.get("status") != "executing" or heist.get("inside_response"):
            return
        await self.bot.repo.update_heist(heist_id, inside_response="[timeout]", inside_success=0)
        guild = await self.bot.city_service.get_guild(heist["guild_id"])  # type: ignore[union-attr]
        member = guild.get_member(heist.get("inside_id"))
        if member is None:
            return
        try:
            await member.send(embed=self.bot.embed_factory.danger("Inside Task Failed", "The 2-second camera window closed before your response."))
        except discord.HTTPException:
            pass
