...
from sinbot.models.game import BlackjackSession
from sinbot.views.arms_deal import ArmsDealView
from sinbot.views.blackjack import BlackjackView
...
class CasinoService:
    ...
    async def rehydrate_wagers(self, guild_id: int) -> None:
        rows = await self.bot.repo.db.execute_fetchall(
            "SELECT user_id, quantity FROM player_inventory WHERE guild_id = ? AND item_name = 'blackjack_wager' AND quantity > 0",
            (guild_id,)
        )
        for row in rows:
            user_id = row["user_id"]
            amount = row["quantity"]
            await self.bot.repo.credit_wallet(guild_id, user_id, amount)
            await self.bot.repo.adjust_inventory(guild_id, user_id, 'blackjack_wager', -amount)
    ...
    async def _validate_player(self, member: discord.Member) -> dict:
        ...
        if player is None or not player["is_joined"]:
            raise InvalidStateError("You need to join the city before hitting the casino.")
    ...
    async def start_blackjack(self, member: discord.Member, bet: int, channel: discord.TextChannel) -> discord.Message:
        ...
        if bet <= 0:
            raise InvalidStateError("Your bet has to be greater than zero.")
        await self.bot.repo.debit_wallet(member.guild.id, member.id, bet)
        await self.bot.repo.adjust_inventory(member.guild.id, member.id, 'blackjack_wager', bet)
    ...
    async def _finalize_blackjack(
        ...
        *,
        ...
        # Clear the escrowed wager from inventory
        await self.bot.repo.adjust_inventory(guild.id, session.user_id, 'blackjack_wager', -session.bet)
