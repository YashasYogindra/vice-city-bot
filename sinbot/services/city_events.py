...
from sinbot.constants import CITY_EVENT_DURATION_HOURS, RACKS_EMOJI
from sinbot.models.events import CityEvent, CityEventEffect, GroqCityEventResult
...
class CityEventDirectorService:
    ...
    async def build_city_event_embed(self, guild_id: int) -> tuple[discord.Embed, discord.File | None]:
        ...
        if self.bot.visual_service is not None:
            ...
            event_gif = {
                "police_sweep": gifs.POLICE_SWEEP,
                "black_market_sale": gifs.BLACK_MARKET_SALE,
                "casino_rush": gifs.CASINO_RUSH,
                "harbor_shipment": gifs.HARBOR_SHIPMENT
            }.get(active.event_key)
            
            if event_gif:
                embed.set_image(url=event_gif)
