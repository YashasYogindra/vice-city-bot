...
from sinbot.constants import HEAT_EMOJI, HEAT_STATUS, RACKS_EMOJI, TURF_EMOJI
...
class VisualService:
    ...
    async def build_profile_card(self, guild_id: int, user_id: int) -> tuple[discord.Embed, discord.File]:
        ...
        embed = self.bot.embed_factory.standard("City Dossier", "Your identity card is on file.")
    ...
    async def _load_avatar(self, member: discord.Member | None, size: int) -> Image.Image:
        ...
        if member is not None:
            try:
                asset_size = 64
                while asset_size < size and asset_size < 4096:
                    asset_size *= 2
                asset_size = max(64, min(4096, asset_size))
                payload = await member.display_avatar.replace(size=asset_size, static_format="png").read()
