...
from sinbot.constants import (
    ...
    BLACK_MARKET_ITEMS,
    VIOLENCE_SWEEP_THRESHOLD,
    VIOLENCE_HOURLY_DECAY,
    BAIL_MINIMUM,
)
from sinbot.exceptions import InvalidStateError
from sinbot.models.cinematic import GroqInformantTipResult, InformantTipSeed
...
class CityService:
    ...
    async def post_news(self, guild_id: int, title: str, description: str, color_kind: str = "standard", image_url: str | None = None) -> None:
        # Run the description through AI narration for cinematic news
        narrated = description
        if self.bot.groq_service is not None:
            try:
                narrated = await self.bot.groq_service.generate_news_narration(
                    title=title, description=description,
                )
            except Exception:
                narrated = description
        await self.bot.repo.add_news_event(guild_id, title, narrated, color_kind)
        ...
        }.get(color_kind, factory.standard)(title, narrated)
        # Add violence tension bar to news embeds
        try:
            violence = int((await self.bot.repo.ensure_guild_settings(guild_id)).get("violence_level", 0))
            bar_filled = min(10, int(10 * violence / VIOLENCE_SWEEP_THRESHOLD))
            bar = f"{'█' * bar_filled}{'░' * (10 - bar_filled)} {violence}/{VIOLENCE_SWEEP_THRESHOLD}"
            embed.set_footer(text=f"🌡️ City Tension: {bar}")
        except Exception:
            pass
    ...
    async def run_hourly_cycle(self, guild_id: int) -> None:
        ...
        await self._decay_violence(guild_id)
    ...
    async def adjust_violence(self, guild_id: int, delta: int) -> None:
        """Adjust the city-wide violence level. If it crosses the threshold, auto-trigger a police sweep."""
        settings = await self.bot.repo.ensure_guild_settings(guild_id)
        current = int(settings.get("violence_level", 0))
        new_level = max(0, current + delta)
        await self.bot.repo.update_guild_settings(guild_id, violence_level=new_level)
        if new_level >= VIOLENCE_SWEEP_THRESHOLD and self.bot.event_service is not None:
            await self.bot.repo.update_guild_settings(guild_id, violence_level=0)
            await self.bot.event_service.trigger_event(guild_id, "police_sweep", announce=True)
            await self.post_news(
                guild_id,
                "🚨 Violence Threshold Breached",
                "The streets have boiled over! City-wide criminal activity triggered an automatic Police Sweep. "
                "All operations are now punished harder while the heat dies down.",
                "danger",
            )

    async def _decay_violence(self, guild_id: int) -> None:
        """Hourly violence decay."""
        settings = await self.bot.repo.ensure_guild_settings(guild_id)
        current = int(settings.get("violence_level", 0))
        if current > 0:
            new_level = max(0, current - VIOLENCE_HOURLY_DECAY)
            await self.bot.repo.update_guild_settings(guild_id, violence_level=new_level)

    async def get_violence_level(self, guild_id: int) -> int:
        """Return current violence level for display."""
        settings = await self.bot.repo.ensure_guild_settings(guild_id)
        return int(settings.get("violence_level", 0))

    async def bail_player(self, guild_id: int, user_id: int, amount: int) -> discord.Embed:
        """Pay Racks to exit jail early."""
        if amount < BAIL_MINIMUM:
            raise InvalidStateError(f"Minimum bail is **{BAIL_MINIMUM}** Racks.")
        jail = await self.bot.repo.get_active_jail_for_user(guild_id, user_id)
        if jail is None:
            raise InvalidStateError("You are not currently jailed.")
        release_at = parse_datetime(jail["release_at"])
        if release_at is None or release_at <= utcnow():
            raise InvalidStateError("Your jail sentence has already expired.")
        remaining_seconds = (release_at - utcnow()).total_seconds()
        bail_cost = max(BAIL_MINIMUM, int(remaining_seconds / 60) * 10)
        if amount < bail_cost:
            raise InvalidStateError(f"Bail costs **{bail_cost}** Racks for your remaining sentence.")
        await self.bot.repo.debit_wallet(guild_id, user_id, bail_cost)
        await self.bot.repo.release_jail_record(jail["id"])
        await self.bot.repo.update_player(guild_id, user_id, jailed_until=None)
        await self.post_news(
            guild_id,
            "Bail Posted",
            f"<@{user_id}> paid **{bail_cost}** Racks and walked out of lockup.",
            "reward",
        )
        return self.bot.embed_factory.reward(
            "Bail Posted",
            f"You paid **{bail_cost}** Racks bail and are now free.",
        )
    ...
    async def join_city(self, member: discord.Member) -> dict[str, Any]:
        ...
        if player and player["is_joined"]:
            raise InvalidStateError("You have already joined the city.")
        ...
        if role and role not in member.roles:
            await member.add_roles(role, reason="SinBot gang assignment")
    ...
    async def set_self_xp_for_testing(self, guild_id: int, user_id: int, xp: int) -> discord.Embed:
        if xp < 0:
            raise InvalidStateError("XP must be zero or greater.")
        player = await self.bot.repo.get_player(guild_id, user_id)
        if player is None or not player.get("is_joined"):
            raise InvalidStateError("You need to join the city first.")
        previous_xp = int(player["xp"])
        previous_rank = str(player["rank"])
        await self.bot.repo.update_player(guild_id, user_id, xp=int(xp))
        updated, _ = await self.recalculate_rank(guild_id, user_id)
        embed = self.bot.embed_factory.success(
            "Testing XP Updated",
            f"Your XP was set from **{previous_xp}** to **{updated['xp']}**.",
        )
        embed.add_field(name="Rank", value=f"{previous_rank} -> {updated['rank']}", inline=True)
        return embed

    async def switch_gang_for_testing(self, member: discord.Member, gang_name: str) -> discord.Embed:
        player = await self.bot.repo.get_player(member.guild.id, member.id)
        if player is None or not player.get("is_joined"):
            raise InvalidStateError("You need to join the city first.")
        target_gang = await self.bot.repo.get_gang_by_name(member.guild.id, gang_name)
        if target_gang is None:
            raise InvalidStateError("That gang does not exist.")

        current_gang = await self.bot.repo.get_gang(player["gang_id"]) if player.get("gang_id") else None
        if current_gang and current_gang["id"] == target_gang["id"]:
            raise InvalidStateError(f"You are already in **{target_gang['name']}**.")

        if current_gang and current_gang.get("boss_user_id") == member.id:
            await self.bot.repo.upsert_gang(member.guild.id, current_gang["name"], boss_user_id=None)

        update_fields: dict[str, Any] = {"gang_id": target_gang["id"], "trust_score": 0}
        if player["rank"] != "Mayor":
            update_fields["rank"] = "Street Rat"
        await self.bot.repo.update_player(member.guild.id, member.id, **update_fields)

        old_role = member.guild.get_role(current_gang["role_id"]) if current_gang and current_gang.get("role_id") else None
        if old_role and old_role in member.roles:
            await member.remove_roles(old_role, reason="SinBot test gang switch")
        new_role = member.guild.get_role(target_gang["role_id"]) if target_gang.get("role_id") else None
        if new_role and new_role not in member.roles:
            await member.add_roles(new_role, reason="SinBot test gang switch")

        updated, _ = await self.recalculate_rank(member.guild.id, member.id)
        old_gang_name = current_gang["name"] if current_gang else "No Gang"
        embed = self.bot.embed_factory.success(
            "Testing Gang Updated",
            f"You moved from **{old_gang_name}** to **{target_gang['name']}**.",
        )
        embed.add_field(name="Rank", value=str(updated["rank"]), inline=True)
        embed.add_field(name="XP", value=str(updated["xp"]), inline=True)
        return embed
    ...
    async def build_profile_embed(self, guild_id: int, user_id: int) -> discord.Embed:
        player = await self.bot.repo.get_player(guild_id, user_id)
        if player is None:
            raise InvalidStateError("That player does not have a city profile yet.")
        guild = await self.get_guild(guild_id)
        member = guild.get_member(user_id)
        gang = await self.bot.repo.get_gang(player["gang_id"]) if player.get("gang_id") else None
        ...
        if player.get("jailed_until"):
            ...
            if release and release > utcnow():
                jail_line = f"Locked up — release {discord.utils.format_dt(release, style='R')}"
        # Trust score
        trust_line = f"Trust: **{player.get('trust_score', 0)}**"
        # Inventory
        inv_parts = [f"{name}={qty}" for name, qty in inventory.items() if qty > 0]
        inv_text = ", ".join(inv_parts) if inv_parts else "Empty"
        description = "\n".join(
            [
                ...
                trust_line,
                f"⚖️ Status: {jail_line}",
                f"Inventory: {inv_text}",
            ]
        )
        embed = self.bot.embed_factory.standard("Profile", description)
        if member:
    ...
    async def build_gang_embed(self, guild_id: int, user_id: int) -> discord.Embed:
        ...
        # Trust scores visible to boss only
        if player["rank"] == "Boss" and members:
            trust_lines = []
            for m in members[:10]:
                mem = guild.get_member(m["user_id"])
                name = mem.display_name if mem else str(m["user_id"])
                trust_lines.append(f"{name}: **{m.get('trust_score', 0)}**")
            embed.add_field(name="🤐 Trust Scores (Boss Eyes Only)", value="\n".join(trust_lines), inline=False)
        if boss is not None:
    ...
    async def build_news_embed(self, guild_id: int, *, limit: int = 10) -> discord.Embed:
        ...
        embed = self.bot.embed_factory.standard("City News", "The city's latest chatter, scoreboards, and blood in the water.")
        if self.bot.event_service is not None:
    ...
    async def build_wanted_embed(self, guild_id: int) -> tuple[discord.Embed, discord.File | None]:
        ...
        wanted = await self.bot.repo.list_wanted_players(guild_id)
        if not wanted:
            embed = self.bot.embed_factory.danger("Wanted Board", "No one is currently on the wanted board.")
            if gifs.MOST_WANTED:
                embed.set_image(url=gifs.MOST_WANTED)
            elif gifs.POLICE_SWEEP:
                embed.set_image(url=gifs.POLICE_SWEEP)
            return embed, None
        lines = []
        ...
        if poster_file is None:
            ...
            embed.set_thumbnail(url=gifs.POLICE_SWEEP)
    ...
    async def build_leaderboard_embed(self, guild_id: int) -> discord.Embed:
        ...
        embed = self.bot.embed_factory.reward("Leaderboard", "The city's loudest names right now.")
        richest_text = "\n".join(
    ...
    def build_shop_embed(self, item_name: str | None = None) -> discord.Embed:
        ...
        for item, details in BLACK_MARKET_ITEMS.items():
            ...
            if selected == item:
                if item == "weapon":
                    value += "\nConsumed in turf wars. Each weapon adds 25% war power."
                elif item == "burnerphone":
                    value += "\nBurns on operations or arms deals to dodge extra Heat."
                elif item == "lawyer":
                    value += "\nInstantly knocks 2 Heat off if your cooldown is clear."
                elif item == "vest":
                    value += "\nAuto-absorbs damage once in your next duel."
                elif item == "scanner":
                    value += "\nConsumed on heist launch to reveal the safest driver route."
                else:
                    value += "\nConsumed on failed heists to avoid jail once."
            embed.add_field(name=label, value=value, inline=True)
        if gifs.BLACK_MARKET_SALE:
            embed.set_image(url=gifs.BLACK_MARKET_SALE)
        elif gifs.MARKET_PURCHASE:
            embed.set_image(url=gifs.MARKET_PURCHASE)
        return embed
    ...
    async def buy_item(self, guild_id: int, user_id: int, item_name: str) -> discord.Embed:
        ...
        usage_line = {
            "weapon": "War boost: consumed in turf wars (+25% power each).",
            "burnerphone": "Operation stealth: consumed to mask your trail and avoid extra Heat.",
            "vest": "Duel armor: auto-absorbs your first incoming hit in /fight.",
            "scanner": "Heist intel: consumed at /heist go to reveal the safest driver route.",
            "medkit": "Heist rescue: consumed on a failed heist to avoid lockup once.",
        }.get(item_name, "Stored in inventory.")
        return self.bot.embed_factory.reward(
            "Black Market Purchase",
            (
                f"You bought **{item_name}** for **{final_price}** Racks. "
                f"Inventory now: **{new_quantity}**.\n{usage_line}"
            ),
        )
