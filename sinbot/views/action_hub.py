...
HELP_SECTIONS: dict[str, tuple[str, str]] = {
    "Getting Started": (
        "Getting Started",
        "1. Use `/join` or `!join` to enter the city.\n"
    ...
    "Turf Wars": (
        ...
        "Any joined gang member can commit to assault or defend, and gang-bank money fuels the whole push.",
    ),
    "Heat & Wanted": (
        "Heat & Wanted",
...
def build_guide_embed() -> discord.Embed:
    embed = discord.Embed(
        title="SinBot Guide",
        description=(
            "SinBot is a single-server gang simulation. You join a crew, run operations, manage Heat, "
...
class OwnerLockedView(discord.ui.View):
    def __init__(self, bot: "SinBot", owner_id: int | None = None, *, timeout: float = 180) -> None:
...
class QuickActionsView(OwnerLockedView):
    def __init__(self, bot: "SinBot", owner_id: int) -> None:
    ...
    async def tip_button(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        elif self.bot.visual_service is not None and media_key:
...
class HelpNavView(OwnerLockedView):
    def __init__(self, bot: "SinBot", owner_id: int | None = None) -> None:
...
class OperateSelectView(OwnerLockedView):
    ...
    async def run_drug(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        is_success = "success" in result.media_key.lower() if result.media_key else False
        if is_success and gifs.DRUG_RUN_SUCCESS:
            result.embed.set_image(url=gifs.DRUG_RUN_SUCCESS)
        elif not is_success and gifs.DRUG_RUN_BUSTED:
            result.embed.set_image(url=gifs.DRUG_RUN_BUSTED)
        elif self.bot.visual_service is not None and result.media_key:
