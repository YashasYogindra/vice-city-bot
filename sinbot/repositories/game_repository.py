...
from sinbot.constants import DEFAULT_TAX_RATE, TURF_HOURLY_INCOME
from sinbot.exceptions import InsufficientFundsError
from sinbot.models.events import CityEvent, CityEventEffect
...
class GameRepository:
    ...
    async def initialize(self) -> None:
        await self.db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS players (
                ...
                operations_success INTEGER NOT NULL DEFAULT 0,
    ...
    async def _ensure_players_columns(self) -> None:
        ...
        if "operations_success" not in existing:
            await self.db.execute("ALTER TABLE players ADD COLUMN operations_success INTEGER NOT NULL DEFAULT 0")
        if "operations_failed" not in existing:
            await self.db.execute("ALTER TABLE players ADD COLUMN operations_failed INTEGER NOT NULL DEFAULT 0")
        await self._ensure_guild_settings_columns()

    async def _ensure_guild_settings_columns(self) -> None:
        rows = await self.db.execute_fetchall("PRAGMA table_info(guild_settings)")
        existing = {str(row["name"]) for row in rows}
        if "violence_level" not in existing:
            await self.db.execute("ALTER TABLE guild_settings ADD COLUMN violence_level INTEGER NOT NULL DEFAULT 0")
