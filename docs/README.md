# SinBot - AI-Directed Discord Crime Sim

SinBot turns one Discord server into a live crime simulation where gangs earn cash, draw Heat, fight over turf, gamble, bribe City Hall, and react to citywide events.

The differentiator is the **City Event Director**: a live event system that rotates the city into states like Police Sweep, Black Market Sale, Casino Rush, and Harbor Shipment. Groq can write the bulletin copy, but every gameplay modifier is deterministic and falls back safely without an API key.
...
The City Event Director lives in `sinbot/services/city_events.py` and is wired into bot startup. On runtime recovery, the bot ensures a city event is active for the configured guild and schedules the next rotation.
...
- Deterministic interrogation scoring system (approach + heat + rank + AI quality)

### Rank System

| Rank | XP Threshold | Perks |
| --- | --- | --- |
| Street Rat | 0 | Basic operations |
| Soldier | 1,500 | Arms deals, leadership challenge votes |
| Capo | 3,000 | 25% reduced operation cooldown |
| Boss | 5,000 | Full gang control, wars, bank management |
| Mayor | N/A | Tax, crackdown, pardon, treasury rewards |
...
| Command | Purpose |
| --- | --- |
| `/join` | Join the city and receive a gang assignment |
...
| `/mayor pardon [member]` | Request a pardon (Mayor approves/declines via DM) |
...
| `/fight <member> [wager]` | PvP combat with optional wager |
| `/bail <amount>` | Post bail to leave jail early |
...
```mermaid
flowchart TD
    Discord["Discord commands and interactions"] --> Cogs["sinbot/cogs"]
    Cogs --> Services["sinbot/services"]
...
- `sinbot/bot.py`: boot, services, scheduler, command sync, runtime recovery
- `sinbot/repositories/game_repository.py`: SQLite tables and transactional persistence
- `sinbot/services/city_events.py`: event catalog, Groq fallback copy, effect helpers, rotation
- `sinbot/services/operations.py`: operation success, payout, and Heat integration
- `sinbot/services/interrogation.py`: deterministic interrogation scoring system
- `sinbot/services/city.py`: shop pricing, news, vault, profiles, daily rewards
- `sinbot/services/casino.py`: casino payout integration
- `sinbot/cogs/mayor.py`: `/city event` and admin trigger command
- `sinbot/cogs/status.py`: demo confidence health check
...
This repo includes a Linux-friendly `Dockerfile`.

```bash
docker build -t sinbot .
docker run --env-file .env sinbot
```
...
- Interrogation outcomes use a deterministic scoring formula (approach + heat + rank + AI quality bonus + small random factor).
